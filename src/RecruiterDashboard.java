...
class RecruiterDashboard extends JFrame {

    private CardLayout cardLayout;
    private JPanel contentPanel;
    private JLabel Home, ViewJob, postjob,recapp;
    ...
    RecruiterDashboard() {
        ...
        JLabel titleLabel = new JLabel("RecruiterDashboard", SwingConstants.CENTER);
        ...
        String recruiterUsername = session.getRecName();
        int recruiterId = session.getUserId();
        String RecEmail = session.getRecEmail();
        //System.out.println(recruiterId);
        // Create "Home" panel using the new HomePanel class
        RecHomePanel homePanel = new RecHomePanel(recruiterUsername);
        Postjob Postjob = new Postjob(recruiterId,RecEmail,recruiterUsername);
        Postedjob Postedjob = new Postedjob(recruiterId,RecEmail);
        ReceivedApplication ReceivedApplication = new ReceivedApplication(recruiterUsername,recruiterId);
        ...
        contentPanel.add(Postedjob, "View Posted Job");
       contentPanel.add(Postjob, "Post Jobs");
        contentPanel.add(ReceivedApplication , "Received application");
        ...
        ViewJob = new JLabel("View Posted Job");
        ViewJob.setFont(new Font("Arial", Font.BOLD, 26));
        ViewJob.setForeground(Color.WHITE);
        ViewJob.setBounds(360, 80, 250, 50);
        joinPanel.add(ViewJob);

        // Add the "View Job" label
        postjob = new JLabel("Post Jobs");
        postjob.setFont(new Font("Arial", Font.BOLD, 26));
        postjob.setForeground(Color.WHITE);
        postjob.setBounds(600, 80, 200, 50);
        joinPanel.add(postjob);
        // Add the "View Job" label
        recapp = new JLabel("Received application");
        recapp.setFont(new Font("Arial", Font.BOLD, 26));
        recapp.setForeground(Color.WHITE);
        recapp.setBounds(790, 80, 320, 50);
        joinPanel.add(recapp);
        ...
        ViewJob.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                resetLabelColors();
                cardLayout.show(contentPanel, "View Posted Job");
                ViewJob.setForeground(Color.CYAN); // Highlight the clicked label
            }
        });

        postjob.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                resetLabelColors();
                cardLayout.show(contentPanel, "Post Jobs"); // Ensure the identifier matches exactly
                postjob.setForeground(Color.CYAN); // Highlight the clicked label
            }
        });

        recapp.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                resetLabelColors();
                cardLayout.show(contentPanel, "Received application");
                recapp.setForeground(Color.CYAN); // Highlight the clicked label
            }
        });
...
// Show the dropdown menu from the right side of the profile button when clicked
        ...
        profile.setText(recruiterUsername);
