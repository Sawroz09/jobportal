...
class CandidateDashboard extends JFrame {

    private CardLayout cardLayout;
    private JPanel contentPanel;
    private JLabel Home, ViewJob,Viewappjob;
    ...
    CandidateDashboard() {
        ...
        JLabel titleLabel = new JLabel("CandidateDashboard", SwingConstants.CENTER);
        ...
        int canid= session.getUserId();
        String userName = session.getCanName();
        String Qualification=session.getQualification();
        String Email=session.getEmail();

        // Create "Home" panel using the new HomePanel class
        CanHomePanel homePanel = new CanHomePanel(userName);
        Canjoblist   canjob=new Canjoblist(userName,Qualification,Email,canid);
        viewapply    viewapp=new viewapply(canid);

        // Create "View Job" panel
        JPanel viewJobPanel = new JPanel(null);
        viewJobPanel.setOpaque(false); // Make the view job panel transparent
        JLabel viewJobLabel = new JLabel("Job Listings Will Be Here!");
        viewJobLabel.setFont(new Font("Arial", Font.BOLD, 36));
        viewJobLabel.setForeground(Color.WHITE);
        viewJobLabel.setBounds(100, 100, 600, 50);
        viewJobPanel.add(viewJobLabel);

        // Add panels to the content panel
        contentPanel.add(homePanel, "Home");
        contentPanel.add(canjob, "ViewJob");
        contentPanel.add(viewapp, "View Applied Job");
        ...
        // Add the "View Job" label
        Viewappjob = new JLabel("View Applied Job");
        Viewappjob.setFont(new Font("Arial", Font.BOLD, 26));
        Viewappjob.setForeground(Color.WHITE);
        Viewappjob.setBounds(560, 80, 250, 50);
        joinPanel.add(Viewappjob);
        ...
        Viewappjob.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                resetLabelColors();
                cardLayout.show(contentPanel, "View Applied Job");
                Viewappjob.setForeground(Color.CYAN); // Highlight the clicked label
...
// Show the dropdown menu from the right side of the profile button when clicked
    ...
    private void resetLabelColors() {
        ...
        ViewJob.setForeground(Color.WHITE);
        Viewappjob.setForeground(Color.WHITE); // Add this line to reset Viewappjob color
